...
export function decodeSubgroupHeader(
  ...
  if (mode === SubgroupIdMode.EXPLICIT) {
    ...
    pos += sgidBytes;
    subgroupId = sgid;
  } else if (mode === SubgroupIdMode.ZERO) {
    // Mode 0: subgroup ID = 0
    subgroupId = varint(0);
  } else {
    // Mode 1: subgroup ID = first object ID (will be set when first object is decoded)
    // Use a placeholder; the caller must update this when decoding the first object
    subgroupId = varint(0);
  }
