...
export function encodeObjectDatagram(datagram: ObjectDatagram): Uint8Array {
  ...
  // §10.3.1: Validate flag/field consistency
  if ((typeByte & DatagramFlags.EXTENSIONS) !== 0 && datagram.extensions == null) {
    throw new Error('EXTENSIONS flag set but extensions data is missing');
  }
  if ((typeByte & DatagramFlags.DEFAULT_PRIORITY) === 0 && datagram.publisherPriority == null) {
...
export function encodeFetchObject(object: FetchObject): Uint8Array {
  ...
  // §10.4.4: Validate flag/field consistency
  if ((numFlags & FetchFlags.PRIORITY) !== 0 && object.publisherPriority == null) {
    throw new Error('PRIORITY flag set but publisherPriority is missing');
  }
  if ((numFlags & FetchFlags.EXTENSIONS) !== 0 && object.extensions == null) {
    throw new Error('EXTENSIONS flag set but extensions data is missing');
  }
